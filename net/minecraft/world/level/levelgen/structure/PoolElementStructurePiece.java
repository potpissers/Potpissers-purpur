...
public class PoolElementStructurePiece extends StructurePiece {
    ...
    public PoolElementStructurePiece(StructurePieceSerializationContext context, CompoundTag tag) {
        ...
        ListTag list = tag.getList("junctions", 10);
        this.junctions.clear();
        list.forEach(junctionTag -> this.junctions.add(JigsawJunction.deserialize(new Dynamic<>(dynamicOps, junctionTag))));
        this.liquidSettings = LiquidSettings.CODEC.parse(NbtOps.INSTANCE, tag.get("liquid_settings")).result().orElse(JigsawStructure.DEFAULT_LIQUID_SETTINGS);
    }
