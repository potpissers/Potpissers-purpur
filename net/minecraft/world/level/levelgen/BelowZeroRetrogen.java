...
public final class BelowZeroRetrogen {
    ...
    public static BelowZeroRetrogen read(CompoundTag tag) {
        ...
        return chunkStatus == ChunkStatus.EMPTY ? null : new BelowZeroRetrogen(chunkStatus, Optional.of(BitSet.valueOf(tag.getLongArray("missing_bedrock"))));
    }
    ...
    public static void replaceOldBedrock(ProtoChunk chunk) {
        ...
        BlockPos.betweenClosed(0, 0, 0, 15, 4, 15).forEach(blockPos -> {
            if (chunk.getBlockState(blockPos).is(Blocks.BEDROCK)) {
                chunk.setBlockState(blockPos, Blocks.DEEPSLATE.defaultBlockState(), false);
            }
        });
    }
        ...
        for (int i = 0; i < 16; i++) {
            for (int i1 = 0; i1 < 16; i1++) {
                if (this.hasBedrockHole(i, i1)) {
                    BlockPos.betweenClosed(i, minY, i1, i, maxY, i1).forEach(blockPos -> chunk.setBlockState(blockPos, Blocks.AIR.defaultBlockState(), false));
                }
            }
        }
    }
    ...
    public static BiomeResolver getBiomeResolver(BiomeResolver resolver, ChunkAccess access) {
        if (!access.isUpgrading()) {
            ...
            return (x, y, z, sampler) -> {
                ...
                return noiseBiome.is(predicate) ? noiseBiome : access.getNoiseBiome(x, 0, z);
            };
        }
    }
}
