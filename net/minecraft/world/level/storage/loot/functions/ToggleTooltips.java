...
public class ToggleTooltips extends LootItemConditionalFunction {
    private static final Map<DataComponentType<?>, ToggleTooltips.ComponentToggle<?>> TOGGLES = Stream.of(
        ...
        .collect(Collectors.toMap(ToggleTooltips.ComponentToggle::type, componentToggle -> (ToggleTooltips.ComponentToggle<?>)componentToggle));
    private static final Codec<ToggleTooltips.ComponentToggle<?>> TOGGLE_CODEC = BuiltInRegistries.DATA_COMPONENT_TYPE
        .byNameCodec()
        .comapFlatMap(
            dataComponentType -> {
                ToggleTooltips.ComponentToggle<?> componentToggle = TOGGLES.get(dataComponentType);
                return componentToggle != null
                    ? DataResult.success(componentToggle)
                    : DataResult.error(
                        () -> "Can't toggle tooltip visiblity for " + BuiltInRegistries.DATA_COMPONENT_TYPE.getKey((DataComponentType<?>)dataComponentType)
                    );
            },
    ...
    private ToggleTooltips(List<LootItemCondition> predicates, Map<ToggleTooltips.ComponentToggle<?>, Boolean> values) {
        super(predicates);
        this.values = values;
    }
    ...
    protected ItemStack run(ItemStack stack, LootContext context) {
        this.values.forEach((componentToggle, _boolean) -> componentToggle.applyIfPresent(stack, _boolean));
        return stack;
    }
    ...
    record ComponentToggle<T>(DataComponentType<T> type, ToggleTooltips.TooltipWither<T> setter) {
        public void applyIfPresent(ItemStack stack, boolean showTooltip) {
            T object = stack.get(this.type);
            if (object != null) {
                stack.set(this.type, this.setter.withTooltip(object, showTooltip));
            }
        }
    }
