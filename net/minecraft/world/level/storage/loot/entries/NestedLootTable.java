...
public class NestedLootTable extends LootPoolSingletonContainer {
    public static final MapCodec<NestedLootTable> CODEC = RecordCodecBuilder.mapCodec(
        instance -> instance.group(
                Codec.either(ResourceKey.codec(Registries.LOOT_TABLE), LootTable.DIRECT_CODEC).fieldOf("value").forGetter(lootTable -> lootTable.contents)
            )
                ...
                resourceKey1 -> validationContext.resolver()
                    ...
                    .ifPresentOrElse(
                        reference -> reference.value()
                            .validate(validationContext.enterElement("->{" + resourceKey1.location() + "}", (ResourceKey<?>)resourceKey1)),
                        () -> validationContext.reportProblem("Unknown loot table called " + resourceKey1.location())
                    )
            )
