...
public abstract class Display extends Entity {
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("interpolation_duration", 99)) {
            int _int = tag.getInt("interpolation_duration");
            this.setTransformationInterpolationDuration(_int);
        }

        if (tag.contains("start_interpolation", 99)) {
            int _int = tag.getInt("start_interpolation");
            this.setTransformationInterpolationDelay(_int);
        }

        if (tag.contains("teleport_duration", 99)) {
            int _int = tag.getInt("teleport_duration");
            this.setPosRotInterpolationDuration(Mth.clamp(_int, 0, 59));
        }

        if (tag.contains("billboard", 8)) {
            Display.BillboardConstraints.CODEC
                .decode(NbtOps.INSTANCE, tag.get("billboard"))
                .resultOrPartial(Util.prefix("Display entity", LOGGER::error))
                .ifPresent(pair -> this.setBillboardConstraints(pair.getFirst()));
        }

        if (tag.contains("view_range", 99)) {
            this.setViewRange(tag.getFloat("view_range"));
        }

        if (tag.contains("shadow_radius", 99)) {
            this.setShadowRadius(tag.getFloat("shadow_radius"));
        }

        if (tag.contains("shadow_strength", 99)) {
            this.setShadowStrength(tag.getFloat("shadow_strength"));
        }
        ...
        if (tag.contains("glow_color_override", 99)) {
            this.setGlowColorOverride(tag.getInt("glow_color_override"));
        }

        if (tag.contains("brightness", 10)) {
            Brightness.CODEC
                .decode(NbtOps.INSTANCE, tag.get("brightness"))
                .resultOrPartial(Util.prefix("Display entity", LOGGER::error))
    ...
    protected void addAdditionalSaveData(CompoundTag tag) {
        Transformation.EXTENDED_CODEC.encodeStart(NbtOps.INSTANCE, createTransformation(this.entityData)).ifSuccess(tag1 -> tag.put("transformation", tag1));
        Display.BillboardConstraints.CODEC.encodeStart(NbtOps.INSTANCE, this.getBillboardConstraints()).ifSuccess(tag1 -> tag.put("billboard", tag1));
        ...
        Brightness brightnessOverride = this.getBrightnessOverride();
        if (brightnessOverride != null) {
            Brightness.CODEC.encodeStart(NbtOps.INSTANCE, brightnessOverride).ifSuccess(tag1 -> tag.put("brightness", tag1));
        }
    }
    ...
    public void lerpTo(double x, double y, double z, float yRot, float xRot, int steps) {
        int posRotInterpolationDuration = this.getPosRotInterpolationDuration();
        this.posRotInterpolationTarget = new Display.PosRotInterpolationTarget(posRotInterpolationDuration, x, y, z, yRot, xRot);
    }
    ...
    public static class ItemDisplay extends Display {
        ...
        protected void readAdditionalSaveData(CompoundTag tag) {
            ...
            if (tag.contains("item")) {
                this.setItemStack(ItemStack.parse(this.registryAccess(), tag.getCompound("item")).orElse(ItemStack.EMPTY));
            } else {
                this.setItemStack(ItemStack.EMPTY);
            }

            if (tag.contains("item_display", 8)) {
                ItemDisplayContext.CODEC
                    .decode(NbtOps.INSTANCE, tag.get("item_display"))
                    .resultOrPartial(Util.prefix("Display entity", Display.LOGGER::error))
                    .ifPresent(pair -> this.setItemTransform(pair.getFirst()));
            }
        }
        ...
        protected void addAdditionalSaveData(CompoundTag tag) {
            ...
            if (!this.getItemStack().isEmpty()) {
                tag.put("item", this.getItemStack().save(this.registryAccess()));
            }

            ItemDisplayContext.CODEC.encodeStart(NbtOps.INSTANCE, this.getItemTransform()).ifSuccess(tag1 -> tag.put("item_display", tag1));
        }
    ...
    public static class TextDisplay extends Display {
        ...
        protected void readAdditionalSaveData(CompoundTag tag) {
            super.readAdditionalSaveData(tag);
            if (tag.contains("line_width", 99)) {
                this.setLineWidth(tag.getInt("line_width"));
            }

            if (tag.contains("text_opacity", 99)) {
                this.setTextOpacity(tag.getByte("text_opacity"));
            }

            if (tag.contains("background", 99)) {
                this.setBackgroundColor(tag.getInt("background"));
            }
            ...
            Optional<Display.TextDisplay.Align> optional = Display.TextDisplay.Align.CODEC
                .decode(NbtOps.INSTANCE, tag.get("alignment"))
                .resultOrPartial(Util.prefix("Display entity", Display.LOGGER::error))
                .map(Pair::getFirst);
            ...
            this.setFlags(b);
            if (tag.contains("text", 8)) {
                String string = tag.getString("text");
        ...
        protected void addAdditionalSaveData(CompoundTag tag) {
            super.addAdditionalSaveData(tag);
            tag.putString("text", Component.Serializer.toJson(this.getText(), this.registryAccess()));
            ...
            storeFlag(flags, tag, "shadow", (byte)1);
            storeFlag(flags, tag, "see_through", (byte)2);
            storeFlag(flags, tag, "default_background", (byte)4);
