...
public class OminousItemSpawner extends Entity {
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        ItemStack itemStack = tag.contains("item", 10)
            ? ItemStack.parse(this.registryAccess(), tag.getCompound("item")).orElse(ItemStack.EMPTY)
            : ItemStack.EMPTY;
    ...
    protected void addAdditionalSaveData(CompoundTag tag) {
        if (!this.getItem().isEmpty()) {
            tag.put("item", this.getItem().save(this.registryAccess()).copy());
        }
