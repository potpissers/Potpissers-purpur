...
public class BuiltInRegistries {
    ...
    private static <T> Registry<T> registerSimpleWithIntrusiveHolders(ResourceKey<? extends Registry<T>> key, BuiltInRegistries.RegistryBootstrap<T> bootstrap) {
    ...
    private static <T extends Registry<?>> void validate(Registry<T> registry) {
        registry.forEach(registry1 -> {
            ...
            if (registry1 instanceof DefaultedRegistry) {
                ResourceLocation defaultKey = ((DefaultedRegistry)registry1).getDefaultKey();
                Validate.notNull(registry1.getValue(defaultKey), "Missing default of DefaultedMappedRegistry: " + defaultKey);
            }
        });
    }
