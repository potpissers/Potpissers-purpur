...
public class LevelUUIDFix extends AbstractUUIDFix {
    ...
    protected TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped(
            ...
            typed -> typed.updateTyped(DSL.remainderFinder(), typed1 -> typed1.update(DSL.remainderFinder(), dynamic -> {
    ...
    private Dynamic<?> updateDragonFight(Dynamic<?> dynamic) {
        return dynamic.update(
            ...
            dynamic1 -> dynamic1.updateMapValues(
                pair -> pair.mapSecond(
                    dynamic2 -> dynamic2.update("DragonFight", dynamic3 -> replaceUUIDLeastMost(dynamic3, "DragonUUID", "Dragon").orElse(dynamic3))
                )
            )
        );
    }
            ...
            dynamic1 -> dynamic1.updateMapValues(
                pair -> pair.mapSecond(
                    dynamic2 -> dynamic2.update(
                        "Players", dynamic3 -> dynamic2.createList(dynamic3.asStream().map(dynamic4 -> createUUIDFromML((Dynamic<?>)dynamic4).orElseGet(() -> {
